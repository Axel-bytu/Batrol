...
handler.before = async function (m) {
  ...
  if (!(id in this.tebakgambar)) return m.reply('El asunto ha terminado')
  if (m.quoted.id == this.tebakgambar[id][0].id) {
    ...
    } else if (similarity(m.text.toLowerCase(), json.result.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*¡Un poco más!*`)
