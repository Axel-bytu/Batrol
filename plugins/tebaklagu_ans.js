...
handler.before = async function (m) {
    ...
    if (!(id in this.tebaklagu)) return m.reply('El asunto ha terminado')
    if (m.quoted.id == this.tebaklagu[id][0].id) {
        ...
        } else if (similarity(m.text.toLowerCase(), json.judul.toLowerCase().trim()) >= threshold) m.reply(`*¡Un poco más!*`)
