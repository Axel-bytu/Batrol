...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  if (!text) throw `_Ingrese el número!_ \nEjemplo:\n\n${usedPrefix + command + ' ' + global.owner[0]}`
  ...
  if (response[users] == 408) throw `El número se marcó recientemente\nSolo se puede ingresar a través de ${usedPrefix}link`
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    let teks = `Atractivo @${jid.split('@')[0]} usando invitación...`
    m.reply(teks, null, {
      contextInfo: {
